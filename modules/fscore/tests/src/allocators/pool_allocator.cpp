...
BOOST_AUTO_TEST_CASE(allocate_out_of_memory)
{
	...
	FS_REQUIRE_ASSERT([&](){pAllocation = pAllocator->allocateSingle();});
...
{
	FS_REQUIRE_ASSERT([&](){pAllocator->allocate(8, 4);});
	FS_REQUIRE_ASSERT([&](){pAllocator->allocate(7, 4);});
	FS_REQUIRE_ASSERT([&](){pAllocator->allocate(4, 8);});
	FS_REQUIRE_ASSERT([&](){pAllocator->allocate(4, 7);});
	FS_REQUIRE_ASSERT([&](){pAllocator->allocate(4, 4);});
	FS_REQUIRE_ASSERT([&](){pAllocator->allocate(7, 7);});
}
