...
BOOST_AUTO_TEST_CASE(valid_construction)
{
	...
	BOOST_CHECK(marker.position == (size_t)pStackData);
	BOOST_CHECK(marker.allocationIndex == 0);

	marker = pStack->getUpper().getMarker();
	BOOST_CHECK(marker.position == (size_t)pStackData + DEFAULT_STACK_MEM_SIZE);
	BOOST_CHECK(marker.allocationIndex == 0);
}
...
BOOST_AUTO_TEST_CASE(get_lower_and_lower)
{
	...
	BOOST_CHECK(lowerBegin.position == (size_t)pStackData);
	BOOST_CHECK(upperBegin.position == (size_t)pStackData + DEFAULT_STACK_MEM_SIZE);
}
